...
def update_current_linear_velocity():
    ...
    if timeFromStart != 0:
        velocityLeftWheel = getters.get_tick_value_in_rad(leftTickCurrent - refTickLeft) * 0.066 * 0.5/timeChange_2lastUpdates
        velocityRightWheel = getters.get_tick_value_in_rad(rightTickCurrent - refTickRight) * 0.066 * 0.5/timeChange_2lastUpdates
...
def update_xposition():
    vAverageTick, thetaAverageTick = getters.get_deadreckon_v_and_angle_averages()
...
def update_graph_data():
    ...
    nearestObject = getters.get_nearest_object() # distance between the robot and nearest object detected. The scan is used
