...
fileList = extract_data("Saved data linear v1.csv")
print(fileList)
...
while  len(xList) - myCounter >0:
    ...
    yList[myCounter] = yList[myCounter]/xList[myCounter] * 0.066 * 0.5 # radius of wheel, rad/s to m/s
    myCounter += 1
    
xList = np.asarray(xList)
yList = np.asarray(yList)
B_spline_coeff = make_interp_spline(xList, yList)
X_Final = np.linspace(xList.min(), xList.max(), 500) #choose the resolution as 50
Y_Final = B_spline_coeff(X_Final)

# Creating line of step function
Y_step = []
...
plt.title("Linear mvt, step 0.2") #set the title of the graph
plt.step(X_Final, Y_step, 'g', where='pre') # step function
plt.grid()
plt.show() #display the graph
'''
# PART TWO: TREATING 'digested' values for angular movement

fileList = extract_data("Saved data angular v2.csv")
#print(fileList)
"""
...
def get_angular_velocity(leftTick, rightTick, theTime):
    if theTime != 0:
        
        vl = tick_to_rad(leftTick - refTickLeft) * 0.066 * 0.5/theTime
        vr = tick_to_rad(rightTick - refTickRight) * 0.066 * 0.5/theTime
        angu_vel = (vr-vl)/0.16 
...
xList = []
yList = []
refTickLeft = (floatList [0][0])
print(refTickLeft)
refTickRight = (floatList [0][1])
print(refTickRight)
for i in floatList:
    yVal = get_angular_velocity(i[0], i[1], i[2]) # take the avg of left and right tick, converts to radian
    yList.append(yVal)
    xList.append(i[2])
...
#     yList[myCounter] = yList[myCounter]/xList[myCounter] # rad/s
#     myCounter += 1
...
yList = np.asarray(yList)
B_spline_coeff = make_interp_spline(xList, yList)
X_Final = np.linspace(xList.min(), xList.max(), 100) #choose the resolution as 50
Y_Final = B_spline_coeff(X_Final)
